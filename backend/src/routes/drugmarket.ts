...
drugMarketRouter.post("/buy/:itemId", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
...
drugMarketRouter.post("/sell/:inventoryId", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    db.update(schema.users)
      .set({ cash: user.cash + cashAwarded, turns: refreshedTurns - 1 })
      .where(eq(schema.users.id, user.id))
      .run();
...
drugMarketRouter.post("/collect", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const user = await db.query.users.findFirst({ where: eq(schema.users.id, req.userId!) });
    if (!user) { res.status(404).json({ error: "User not found" }); return; }
