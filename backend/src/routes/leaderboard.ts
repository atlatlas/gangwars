...
leaderboardRouter.get("/:type", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    switch (type) {
      ...
      case "networth": {
        // Net worth = cash + respect * 10 + inventory item values
        rows = db.select({
          id: schema.users.id,
          username: schema.users.username,
          level: schema.users.level,
          netWorth: sql<number>`${schema.users.cash} + ${schema.users.respect} * 10 + COALESCE((
            SELECT SUM(${schema.items.buyPrice} * ${schema.userInventory.quantity})
            FROM ${schema.userInventory}
            JOIN ${schema.items} ON ${schema.userInventory.itemId} = ${schema.items.id}
            WHERE ${schema.userInventory.userId} = ${schema.users.id}
          ), 0)`,
        })
          .from(schema.users)
          .orderBy(desc(sql`${schema.users.cash} + ${schema.users.respect} * 10 + COALESCE((
            SELECT SUM(${schema.items.buyPrice} * ${schema.userInventory.quantity})
            FROM ${schema.userInventory}
            JOIN ${schema.items} ON ${schema.userInventory.itemId} = ${schema.items.id}
            WHERE ${schema.userInventory.userId} = ${schema.users.id}
