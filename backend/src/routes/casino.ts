...
casinoRouter.post("/blackjack/deal", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
...
casinoRouter.post("/blackjack/action", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { gameId, action } = req.body;
    if (!gameId || !["hit", "stand"].includes(action)) {
    ...
    if (action === "hit") {
      ...
      if (pv > 21) {
        ...
        // Player busts — no payout (already deducted)
    ...
    bs.playerDone = true;
    let dd = bs.deck;
    while (handValue(bs.dealerHand) < 17) {
...
casinoRouter.post("/slots/spin", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const bet = Math.floor(req.body?.bet ?? 0);
    if (bet < 100) { res.status(400).json({ error: "Minimum bet is $100" }); return; }
...
casinoRouter.post("/ride-the-bus/deal", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const bet = Math.floor(req.body?.bet ?? 0);
    if (bet < 100) { res.status(400).json({ error: "Minimum bet is $100" }); return; }
...
casinoRouter.post("/ride-the-bus/action", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
