...
hoesRouter.get("/", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Count pimps owned
    const pimpRows = db.select()
      .from(schema.userInventory)
      .innerJoin(schema.items, eq(schema.userInventory.itemId, schema.items.id))
      .where(and(eq(schema.userInventory.userId, user.id), eq(schema.items.type, "pimp")))
      .all();
    const pimpCount = pimpRows.reduce((sum, r) => sum + r.user_inventory.quantity, 0);
    const hoeCount = rows.length;
    const protectedCapacity = pimpCount * 5;
    const unprotectedHoes = Math.max(0, hoeCount - protectedCapacity);
...
hoesRouter.post("/collect", authMiddleware, jailCheck, hpCheck, async (req: AuthRequest, res: Response) => {
  try {
    const user = await db.query.users.findFirst({ where: eq(schema.users.id, req.userId!) });
    if (!user) { res.status(404).json({ error: "User not found" }); return; }
    ...
    if (totalCollected >= 5000) {
      logActivityEvent(user.id, "hoe_collected",
        ...
        { totalCollected, hoeCount: results.length });
    }

    // ─── Kidnapping check: 1 pimp protects 5 hoes ───
    const pimpRows = db.select()
      .from(schema.userInventory)
      .innerJoin(schema.items, eq(schema.userInventory.itemId, schema.items.id))
      .where(and(eq(schema.userInventory.userId, user.id), eq(schema.items.type, "pimp")))
    ...
    const kidnapped: string[] = [];
    if (unprotectedHoes > 0) {
      // Shuffle and pick which unprotected hoes get targeted
      const unprotected = [...rows].sort(() => Math.random() - 0.5).slice(0, unprotectedHoes);
      for (const row of unprotected) {
        if (Math.random() < 0.08) { // 8% chance per unprotected hoe
          ...
          kidnapped.push(row.item.name);
        }
      }

      if (kidnapped.length > 0) {
        logActivityEvent(user.id, "hoe_kidnapped",
          `${kidnapped.length} hoe${kidnapped.length > 1 ? "s" : ""} got kidnapped! Need more pimps to protect them.`,
          { kidnapped: kidnapped.join(", "), pimpCount, hoeCount, unprotectedHoes });
      }
    }
...
hoesRouter.post("/buy/:itemId", authMiddleware, jailCheck, hpCheck, async (req: AuthRequest, res: Response) => {
  try {
