            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                {user.hp < user.maxHp && user.hp > 0 && (
                  <button onClick={() => handleHeal("cash")} disabled={healing} className="font-mono tracking-wider text-[9px] uppercase text-green-300 hover:text-green-200 border border-green-400/40 hover:border-green-400/60 rounded-sm px-1.5 py-0.5 transition-all flex items-center gap-0.5"><Heart size={8} /> ${((user.maxHp - user.hp) * 2).toLocaleString()}</button>
                )}
                {user.hp <= 0 && (
                  <>
                    <button onClick={() => handleHeal("cash")} disabled={healing} className="font-mono tracking-wider text-[9px] uppercase text-green-300 hover:text-green-200 border border-green-400/40 hover:border-green-400/60 rounded-sm px-1.5 py-0.5 flex items-center gap-0.5"><Heart size={8} /> Heal</button>
